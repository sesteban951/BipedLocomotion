...
class HLIPTrajectoryGeneratorSE2():
    ...
    def __init__(self, model_file):
        ...
        self.uy_max = 0.4
        self.u_L_bias =  0.2  # left is swing foot
        self.u_R_bias = -0.2  # right is swing foot
        ...
        self.tol_base = np.array([[np.inf], [epsilon_base], [epsilon_base]])
        self.tol_feet = np.array([[np.inf], [epsilon_feet], [epsilon_feet]]) 
    ...
    def compute_execution_solutions(self, L, I):
        ...
        for k in range(len(L)):
            ...
            x0 = np.array([py_R, vy_R]).reshape(2,1)
            time_set = I[k]
            ...
            py_R_minus = xt_minus[0][0]
            vy_R_minus = xt_minus[1][0]
            ...
            if self.swing_foot_frame.name() == "left_foot":
                u_star = self.u_L_bias + self.v_des * (self.T_SSP + self.T_DSP)
            elif self.swing_foot_frame.name() == "right_foot":
                u_star = self.u_R_bias + self.v_des * (self.T_SSP + self.T_DSP)
            ...
            if k == 0:
                ...
                p_swing_target = p_stance + np.array([0, uy, 0]).reshape(3,1)
            else:
    ...
    def generate_trajectory(self, q0, v0, v_des, t_phase, initial_swing_foot_pos, stance_foot_pos, initial_stance_foot_name):
        ...
        # set P2 orbit parameters
        self.d2 = self.lam**2 * (self.sech(0.5 * self.lam * self.T_SSP))**2 * (self.T * self.v_des) / (self.lam**2 * self.T_DSP + 2 * self.sigma_P2)
        ...
        for i in L:
            ...
            for t, k in zip(time_set, range(len(time_set))):
                ...
                p_R = xt_R[k][0][0]
                p_com_pos =  p_stance + np.array([0, p_R, self.z_nom]).reshape(3,1)
