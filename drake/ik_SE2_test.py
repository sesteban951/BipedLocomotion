...
class Controller(LeafSystem):
    ...
    def CalcOutput(self, context, output):
        ...
        # p_right_des = np.array([[0.1], [0], [0.3]])
        # p_left_des = np.array([[0.1], [0], [0.3]])
        c = np.array([0.1, 0, 0.3])
        r = 0.1
        rps = 1
        omega = -2*np.pi*rps
        phi = np.pi
        p_right_des = np.array([[c[0] + r*np.cos(omega*self.t_current)], [0], [c[2] + r*np.sin(omega*self.t_current)]])
        p_left_des = np.array([[c[0] - r*np.cos(omega*self.t_current)], [0], [c[2] - r*np.sin(omega*self.t_current)]])

        # draw spehere at desired foot positions
        self.meshcat.SetTransform("p_right", RigidTransform([p_right_des[0][0], -0.1, p_right_des[2][0]]), self.t_current)        
        self.meshcat.SetTransform("p_left", RigidTransform([p_left_des[0][0], 0.1, p_left_des[2][0]]), self.t_current)
        ...
        q_des = np.array([q_ik[0], q_ik[1], q_ik[2],  # left leg: hip_pitch, knee, ankle
                          q_ik[3], q_ik[4], q_ik[5]]) # right leg: hip_pitch, knee, ankle
...
# load model
model_file = "../models/achilles_SE2_drake_ik.urdf"
...
# configuration 
q0 = np.array([0, 0, 0,  # left leg: hip_pitch, knee, ankle 
