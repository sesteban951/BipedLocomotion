...
class ReferenceTrajectory:
    ...
    def __init__(self, config, path_num=0):

        #  get the model type (full or half)
        self.model_type = config['model']['type']

        # load reference trajectory parameters
        references = config['reference']['path_list']
        reference_path = references[path_num]
        print("Loading reference trajectory from: {}".format(reference_path))
